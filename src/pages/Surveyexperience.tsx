/**
 * SurveyExperience — the single source of truth for what a respondent sees.
 *
 * Every screen and input the customer-facing survey can render lives here:
 * the welcome/cover screen, the paginated question card, the CAPTCHA step,
 * the completion screen, and every per-question-type input control.
 *
 * `SurveyTakePage` (the live, customer-facing page) wraps this with real
 * data-loading, CAPTCHA verification and submission against the API.
 * The Builder's `PreviewModal` wraps the exact same components with local,
 * in-memory state instead — so what a survey designer sees in "Preview" is
 * pixel-for-pixel what respondents will see once the survey is published.
 */
...
import { evaluateConditionGroup } from '@/utils/logic';
import type { Question, Survey, Branding } from '@/types';
...
// so far, exactly the way the builder's Logic tab evaluates it.
...
export function validateField(
  ...
  if (question.type === 'file-upload' && value && typeof value === 'object') {
    ...
    const allowedFileTypes = (question as any).allowedFileTypes as
      | string[]
      | undefined;
    if (
      maxFileSize &&
...
export function validatePageQuestions(
  questions: Question[],
  answers: Record<string, any>,
  shouldDisplayQuestion: (q: Question, answers: Record<string, any>) => boolean
...
export function hexToRgba(hex: string, alpha: number): string {
  const clean = (hex || '#0ea5e9').replace('#', '');
...
export function pageBackgroundFor(primary: string, accent: string): string {
  return `radial-gradient(1200px circle at 15% -10%, ${hexToRgba(primary, 0.1)}, transparent 45%), radial-gradient(900px circle at 100% 0%, ${hexToRgba(accent, 0.1)}, transparent 40%), #f8fafc`;
}

/* ---------- Brand mark & footer (shared across every screen) ---------- */
...
export function PoweredByFooter({ branding }: { branding: Branding }) {
  if (branding.hidePulseBranding) return null;
  return (
  ...
  return (
    <div className="relative">
      ...
      <select
        ...
        className="appearance-none pl-7 pr-7 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-400 transition cursor-pointer"
      >
        {languages.map((lang) => (
  ...
  return (
    <div className="flex items-center gap-1">
      {Array.from({ length: total }, (_, i) => (
        <div
          key={i}
          className="flex-1 h-1.5 rounded-full bg-slate-200/70 overflow-hidden"
        >
          <div
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl shadow-slate-900/5 border border-slate-100 p-8 sm:p-10 text-center animate-slide-up">
        ...
        <h1 className="text-xl font-semibold font-display text-slate-900 mb-2">
          Survey not found
        </h1>
        <p className="text-sm text-slate-500 leading-relaxed">
          The survey link you followed may have expired or is no longer
          available. Please check the link and try again.
        </p>
      </div>
    </div>
  );
...
export function AlreadyCompletedScreen({
  ...
  const completion =
    survey.completionMessage?.[language] ||
    survey.completionMessage?.[survey.defaultLanguage];
  const title = completion?.title || 'Already completed';
  const body =
    completion?.body ||
    'Thank you for your response. This survey has already been submitted.';
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(180deg, ${hexToRgba(primaryC, 0.06)} 0%, #f8fafc 40%)`,
      }}
    >
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl shadow-slate-900/5 border border-slate-100 p-8 sm:p-10 text-center animate-slide-up">
        <div
...
export function SubmittingScreen({ branding }: { branding: Branding }) {
  const primary = branding.primaryColor;
  return (
    <div
      className="min-h-screen flex items-center justify-center"
      style={{
        background: pageBackgroundFor(primary, branding.accentColor),
            ...
            <div className="relative">
              ...
              {survey.description && (
                <p className="text-sm text-slate-500 leading-relaxed mt-2">
                  {tr('description', survey.description)}
                </p>
              )}
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: `0 10px 25px -5px ${hexToRgba(primary, 0.45)}`,
                }}
              >
    ...
    >
      <div className="max-w-lg w-full">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-900/5 border border-slate-100 p-6 sm:p-8 animate-slide-up">
          ...
          <h2 className="text-xl font-semibold font-display text-slate-900 mb-2">
            Almost done!
          </h2>
          ...
          <div className="flex items-center justify-between gap-3 pt-2">
            ...
            <button
              ...
              style={{
                backgroundColor: primary,
                boxShadow: `0 8px 20px -6px ${hexToRgba(primary, 0.4)}`,
              }}
            >
              {captchaLoading ? (
                <>
...
export function CompletedScreen({
  ...
  return (
    <div
      className={`${heightClass} flex items-center justify-center px-4 py-10`}
      dir={dir}
      style={{
        background: pageBackgroundFor(primary, accent),
        fontFamily: resolveFontFamily(branding.fontFamily),
      }}
    >
      <div className="max-w-lg w-full bg-white rounded-3xl shadow-xl shadow-slate-900/5 border border-slate-100 p-8 sm:p-12 text-center animate-slide-up relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="text-2xl sm:text-3xl font-semibold font-display text-slate-900 mb-3">
            {title}
          </h1>
          ...
          {redirectUrl && (
            <a
              ...
              style={{
                backgroundColor: primary,
                boxShadow: `0 10px 25px -5px ${hexToRgba(primary, 0.4)}`,
              }}
            >
              Continue
...
/**
 * The paginated question card — sticky brand/progress header, the question
 * page itself, and Back/Next navigation. This is the screen respondents
 * spend nearly all their time on, and the one the Builder's preview most
 * needed to match exactly.
 */
export function SurveyPageCard({
  survey,
  currentPage,
  currentPageIndex,
  totalPages,
  ...
  const progress =
    totalPages > 0 ? ((currentPageIndex + 1) / totalPages) * 100 : 100;
  const visibleQuestions =
    currentPage?.questions.filter((q) => shouldDisplayQuestion(q, answers)) ||
    [];
            ...
            <div className="space-y-8 mt-1">
              ...
              {visibleQuestions.length === 0 && (
                <p className="text-sm text-slate-400 italic">
                  This page has no visible questions.
                </p>
              )}
            </div>
...
/* =====================================================================
   Question renderer & per-type inputs
   ===================================================================== */
    ...
    <div className="space-y-3.5">
      <div>
        <label className="block text-[15px] font-semibold text-slate-800 leading-snug">
          ...
          {required && (
            <span
              className="inline-block w-1.5 h-1.5 rounded-full bg-red-400 ml-1.5 align-middle"
              title="Required"
            />
          )}
        </label>
...
export function QuestionInput({
  ...
  switch (question.type) {
    ...
    case 'long-text':
      return (
        <LongTextInput question={question} value={value} onChange={onChange} />
      );
    case 'date':
      return (
        <DateInput question={question} value={value} onChange={onChange} />
      );
    case 'datetime':
      return (
        <DateTimeInput question={question} value={value} onChange={onChange} />
      );
    case 'time':
      return (
        <TimeInput question={question} value={value} onChange={onChange} />
      );
    case 'file-upload':
      return (
        <FileUploadInput
    ...
    case 'address':
      return (
        <AddressInput question={question} value={value} onChange={onChange} />
      );
    case 'location':
      return (
        <LocationInput
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: selected
          ? `0 0 0 1px ${primaryColor}, 0 4px 12px -4px ${hexToRgba(primaryColor, 0.35)}`
          : 'none',
      }}
    >
      {letter && (
      ...
      <span
        className={`flex-1 ${selected ? 'text-slate-900 font-medium' : 'text-slate-700'}`}
      >
        {children}
      </span>
    </button>
          ...
          return (
            <button
              ...
              style={{
                ...
                boxShadow: selected
                  ? `0 4px 12px -4px ${hexToRgba(primaryColor, 0.5)}`
                  : 'none',
              }}
            >
              {n}
        ...
        <thead>
          <tr className="bg-slate-50/80">
            ...
            {columns.map((col, cIdx) => (
              <th
                key={col}
                className="text-center text-xs font-medium text-slate-600 p-3 min-w-[80px]"
              >
    ...
    <div>
      ...
      {fileName ? (
        ...
        >
          <UploadCloud
            className="w-7 h-7"
            style={{ color: dragOver ? primaryColor : '#94a3b8' }}
          />
      ...
      {emojis.map((emoji, i) => (
        <button
          ...
          style={{
            ...
            backgroundColor:
              current === i + 1 ? hexToRgba(primaryColor, 0.08) : 'white',
          }}
        >
          {emoji}
        </button>
        ...
        return (
          <button
            ...
            style={{
              borderColor: selected ? primaryColor : '#e2e8f0',
              boxShadow: selected
                ? `0 0 0 1px ${primaryColor}, 0 8px 20px -6px ${hexToRgba(primaryColor, 0.4)}`
                : 'none',
            }}
          >
            ...
            <p
              className={`text-xs py-2.5 text-center font-medium ${selected ? 'text-slate-900' : 'text-slate-600'}`}
            >
              {label}
            </p>
          </button>
    ...
    return (
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1.5">
          This will be recorded as your signature.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <input
        ...
        className="input !rounded-xl"
        value={v.street || ''}
        onChange={(e) => onChange({ ...v, street: e.target.value })}
      />
      <div className="grid grid-cols-2 gap-3">
        <input
          type="text"
  ...
  return (
    <div className="space-y-5">
      {question.options?.map((pair, pIdx) => {
        const nextPair =
          question.options?.[(pIdx + 1) % (question.options?.length || 1)];
        return (
          <div key={pair.id}>
            ...
            <input
              ...
              value={v[pair.id] ?? 4}
              onChange={(e) =>
                onChange({ ...v, [pair.id]: Number(e.target.value) })
              }
              className="w-full"
